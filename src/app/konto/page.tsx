...
export default function KontoPage() {
  const [data, setData] = useState<KontoData>({ checked_at: null, checks: [], problems_count: 0 });
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/konto-check", { cache: "no-store" });
    ...
    <div className="relative z-10 w-full mx-auto pb-16">
        ...
        <div>
          ...
          <p className="text-[13.5px] text-[var(--text-3)] mt-2 max-w-[65ch]">
            {data.checked_at
              ...
              : "Kontode seisu pole veel kontrollitud."}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <SectionHeader
            label="Kontrollid"
            title={`${data.checks.length} kontot jälgimisel`}
            action={
